...
public class FSEntityResolver implements EntityResolver2 {
    ...
    private FSEntityResolver() {
        ...
        entities.putAll(catalog.parseCatalog("schema/html-4.01/catalog-html-4.01.xml"));
        
        // XHTML common (shared declarations)
        entities.putAll(catalog.parseCatalog("schema/xhtml/catalog-xhtml-common.xml"));

        // The XHTML 1.0 DTDs
        entities.putAll(catalog.parseCatalog("schema/xhtml/catalog-xhtml-1.0.xml"));
        
        // The XHMTL 1.1 DTD
        entities.putAll(catalog.parseCatalog("schema/xhtml/catalog-xhtml-1.1.xml"));
        
        // DocBook DTDs
        entities.putAll(catalog.parseCatalog("schema/docbook/catalog-docbook.xml"));
    ...
    private static InputSource newHTML5DoctypeSource() {
        InputStream io = Optional.ofNullable(FSEntityResolver.class
            .getClassLoader().getResourceAsStream("schema/html5/entities.dtd"))
            .orElseThrow(() -> new IllegalStateException("Could not find " +
                "schema/html5/entities.dtd on the classpath"));
        return new InputSource(io);
    }
